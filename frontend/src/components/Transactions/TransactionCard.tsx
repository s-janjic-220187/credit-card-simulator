...
const TransactionCard: React.FC<TransactionCardProps> = ({
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'CASH_ADVANCE': return 'bg-orange-100 text-orange-800';
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'GROCERIES': return 'bg-green-50 text-green-700';
      case 'DINING': return 'bg-red-50 text-red-700';
      case 'GAS': return 'bg-yellow-50 text-yellow-700';
      case 'ENTERTAINMENT': return 'bg-purple-50 text-purple-700';
      case 'SHOPPING': return 'bg-pink-50 text-pink-700';
      case 'TRAVEL': return 'bg-blue-50 text-blue-700';
      case 'HEALTHCARE': return 'bg-indigo-50 text-indigo-700';
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between">
        <div className="flex items-start space-x-3 flex-1">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
              <span className={`inline-flex items-center px-2 py-1 rounded-full font-medium ${getCategoryColor(transaction.category)}`}>
        ...
        <div className="text-right ml-4">
          ...
          {showActions && transaction.status !== 'CANCELLED' && (onEdit || onDelete) && (
